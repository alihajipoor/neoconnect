...
/// endpoint tables. That lookup can come back empty, and when it does,
/// `Selection::tunnel_when_owner_unknown` is false in `OnlySelected` --
/// so an unattributable packet is **left alone**, which for IPv6 means
/// it goes out in clear text. Two measured ways to get there:
///
