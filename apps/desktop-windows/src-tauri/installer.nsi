; ---------------------------------------------------------------------
; Neoxify's installer, forked from Tauri's own template.
;
; Upstream: tauri-apps/tauri, tag tauri-cli-v2.11.4,
; crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi
;
; Keep this a small, readable diff against that file. When Tauri is
; upgraded, re-fetch the template for the new tag and re-apply the four
; changes below rather than porting the whole thing forward -- the parts
; that are easy to get wrong (the file and resource lists, the upgrade
; and uninstall paths) are exactly the parts nothing here touches.
;
; WHAT IS CHANGED, and nothing else:
;
;  1. A custom first page, `NeoxifyWelcome`: one branded screen with one
;     Install button, in place of the five-page next-next-next wizard.
;  2. That page sets $PassiveMode, which is upstream's own flag for
;     "skip the wizard". Every other page then skips itself through the
;     `SkipIfPassive` calls that already existed. No page was deleted,
;     no install logic was touched.
;  3. A GUI-init hook restyles the window dark and hides MUI's header,
;     and the progress page hides its log so the two pages look like one
;     continuous window.
;  4. $LaunchAfterInstall, so the app opens when it finishes -- upstream
;     only does that for a `/R` command line, and there is no finish
...
; 0. Neoxify's own page. See the header comment: this is the whole
;    interactive part of the installer, and clicking Install on it sets
;    $PassiveMode so every page below skips itself.
Var NeoxifyDirLabel
Var LaunchAfterInstall
; MUI owns `.onGUIInit`, so the window restyling hangs off its hook
; rather than defining that function a second time. Declared here, next
; to the page it styles, so the two are not separated by 900 lines.
!define MUI_CUSTOMFUNCTION_GUIINIT NeoxifyGuiInit
Page custom NeoxifyWelcome NeoxifyWelcomeLeave
...
; 7. Installation page
!define MUI_PAGE_CUSTOMFUNCTION_SHOW NeoxifyInstFilesShow
...
; Neoxify's own strings. Declared after the language files so they exist
; for every language the bundle ships; the English text is used as the
; fallback for languages we have not translated, which is the same thing
; MUI does for anything it does not have.
LangString NxTagline           ${LANG_ENGLISH} "Private, fast, and hard to block."
LangString NxInstall           ${LANG_ENGLISH} "Install"
LangString NxInstalling        ${LANG_ENGLISH} "Installing..."
LangString NxChangeLocation    ${LANG_ENGLISH} "Change location"
LangString NxChooseFolderTitle ${LANG_ENGLISH} "Where should Neoxify be installed?"


Function .onInit
  ; Unpacked before any page runs, because the first one shows it.
  InitPluginsDir
  File "/oname=$PLUGINSDIR\installer-logo.bmp" "${NX_LOGO}"
...
Function .onInstSuccess
  ; Neoxify: opens the app when a person installed it themselves.
  ;
  ; Upstream only launches on an explicit `/R`, because its GUI flow ends
  ; on a finish page with a "run now" checkbox. There is no finish page
  ; here, so without this the installer would simply vanish and leave
  ; the customer to go and find the app.
  ;
  ; Deliberately not done for a genuinely silent install (`/S`), which is
  ; how an administrator deploys this across machines -- windows opening
  ; on other people's desktops is the opposite of what they asked for.
...
Function NeoxifyGuiInit
  ...
  !insertmacro NxHideParent 1036
  !insertmacro NxHideParent 1037  ; header title
  !insertmacro NxHideParent 1038  ; header subtitle
  !insertmacro NxHideParent 1039  ; the white band behind them
  !insertmacro NxHideParent 1045
  !insertmacro NxHideParent 1046  ; header icon
  !insertmacro NxHideParent 1256  ; rule above the buttons
...
Function NeoxifyWelcome
  ...
  ${NSD_OnClick} $R0 NeoxifyChooseFolder
...
Function NeoxifyChooseFolder
  ...
  ${If} $R1 != error
    StrCpy $INSTDIR $R1
    ${NSD_SetText} $NeoxifyDirLabel "$INSTDIR"
  ${EndIf}
FunctionEnd

Function NeoxifyInstallClicked
  Pop $R0
  ; Guards a second click while the first is still being handled. The
  ; install takes a moment to start, and a double-click would otherwise
  ; post two page transitions.
  EnableWindow $NxInstallButton 0
  ${NSD_SetText} $NxInstallButton "$(NxInstalling)"
  SendMessage $HWNDPARENT ${WM_COMMAND} 1 0
FunctionEnd

Function NeoxifyWelcomeLeave
  ; Upstream's own flag for "the customer has answered everything, get
  ; on with it". Setting it here is what makes every page after this one
  ; skip itself, through the SkipIfPassive calls that were already
  ; there -- no page had to be deleted to build this.
  StrCpy $PassiveMode 1
  StrCpy $LaunchAfterInstall 1

  ; The reinstall page reads a radio-button state from $R2, and with the
  ; page skipped that control was never created. Zeroing it makes the
  ; read deterministic instead of whatever happened to be in the
  ; register: 0 means install over the top when upgrading, and a clean
  ; uninstall-then-install when the same version is already there.
  StrCpy $R2 0
FunctionEnd
