...
## 2026-08-26 — Exits have names now, and the reason they did not is not what it looked like

**Status:** done, branch `claude/exit-identity` (off the 0932 integration
head `9137eee`, not merged)
**Touches:** `apps/backend/src/modules/routes/**`, `config/configuration.ts`,
`customer.controller.ts`, `apps/desktop-windows/src/**`, `installer/lib/panel.sh`,
`infra/**`, `docs/design/per-game-exits.md`

Per-game exits shipped unusable and the design doc said why in its own §6:
`GameExitGroup.exit` was `null` everywhere because a route id is not an
exit. That is the whole entry — but the useful part is the *reason* the
backend withheld one, because it nearly sent this the wrong way.

### The reason was not opsec, and checking mattered

Two plausible readings, and they call for different identifiers. Reading
the code rather than assuming: `listAvailableForPlan` uses an explicit
`select` because a plain `include` returns the raw `Route` row **with
`uplinkCredentialsJson` on it** — the relay's shared exit-node secret.
The exit node was excluded alongside it, and the comment's own
justification is that the entry is what the client dials anyway. That is
one narrow correct refusal plus "nobody needed it". Nothing anywhere
argued a customer must not tell two exits apart.

By that reading a plain exit id would have been fine. **It is still
wrong**, and the argument comes from a different document entirely:
`docs/node-address-hygiene.md`'s measurement that enumerability, not ASN
or provider, is what earns an `is_vpn` label. A globally stable exit id
is a fleet identifier — two accounts are enough to count the exits and
join sightings of one across unrelated customers.

**Worth keeping as a habit:** the code's stated reason and the right
answer were not the same thing here. Reading the reason still mattered,
because it ruled *out* the shape that would have been wrong for a
different reason.

### What the handle is

`HMAC(HMAC(EXIT_HANDLE_SECRET, "neoxify:exit-handle:v1"), customerId ‖
exitNodeId)`, 128 bits, base64url. Exit node = `exitProtocolConfig.nodeId`
for a relay, `entryProtocolConfig.nodeId` for a direct route.

Two traps that are only obvious once stated:

* **Rotation is safe in one direction only, and that is the good one.** A
  stale handle cannot collide with a fresh one, so it can produce
  `Fallback` but never a false `OnPreferred`. Cost of rotating the
  secret is that every customer re-picks. Noted in `backup-restore.md`.
* **No secret ⇒ `null`, never an unkeyed digest.** Unkeyed is the same
  string for every customer, which is the single property this must not
  have. Falling back to the previous behaviour beats a weaker version of
  the new one.

`EXIT_HANDLE_SECRET` is generated by the installer and falls back to
`CREDENTIALS_ENCRYPTION_KEY` under its own label, so existing boxes get
the feature without an ops step.

### The client trap that cost the most thought

Grouping routes by handle is easy. **Labelling** the group is not. For a
relay-only exit the obvious label is the relay's node name — and that
name is the *entry*. Using it tells a customer their traffic appears from
Iran when it appears from Germany. `ExitOption.directNames` is built from
non-relay routes only, and a relay-only exit is marked `hidden` and
labelled as what it is reached through, with no claim about where it is.

Second one: `egress` must come from the route the ladder **landed** on,
not the one on screen. Those come apart routinely — nothing pinned, the
ladder orders by speed — and this is the same seam as the 0.9.3 "showed
Singapore, connected to France" bug, one layer up.

### Left undecided on purpose

**Whether a plan gets more than one exit at once is still the owner's
call**, and nothing here decides it. What exists is preference applied on
connect. Also deliberately *not* built: automatic steering. Making the
ladder prefer a route matching a game's chosen exit means choosing which
of two games wins, which is policy. Today a `Fallback` row says which
exit was asked for and the customer switches server themselves.

### Not proven

* **No database was reachable** — the HTTP round-trip spec runs against a
  Prisma stand-in that applies `select` faithfully. The projection is
  pinned; the query is not.
* Nothing has run on a machine. Whether two routes sharing a handle
  really egress from one address is a fleet fact, and §7's rig table is
  still the only thing that would establish it. **Row 1 of that table now
  has something to test** — before this there was no way to express a
  preference at all.
* The picker has not been seen rendered. It typechecks, its pure parts
  are unit-tested, and the card was not opened on a running app.

---

## 2026-08-26 — Three branches onto main, and the one failure that only existed in the combination

Merged in order: `claude/exit-group-guard`, `claude/engine-dns-consistency`,
`claude/exit-identity`. Verified on the merged tree, not on the branches.

**The combination failure, for the fourth time in this repo's history.** Each
branch was green alone and `git merge` reported no conflict outside the
journal. `split-tunnel.persisted-shape.test.ts`, added by `exit-group-guard`,
pins the `vpn_set_split_tunnel` payload to an exact key set — the assertion
that a smuggled `appExits` cannot reach the service merely by riding along.
`exit-identity` adds a sixth key, `egress`, to that same payload. Nothing
either branch could have caught: the test and the field never coexisted until
the merge.

Fixed by extending the pin to six keys, **not** by loosening it to a
containment check — loosening is the one edit that would let through exactly
what the file exists to refuse. `egress` is legitimate there: it is a
parameter of `pushSplitTunnel`, not a field of `SplitTunnelSettings`, so it is
one value for the whole tunnel and cannot express a per-binary split. Its
value semantics are asserted separately in `split-tunnel-egress.test.ts`; what
the guard test asserts is only the key set.

**Both ban-safety invariants re-proven by mutation on the merged tree**, since
a merge is where such guarantees quietly die:

* Adding `appExits?: AppExit[]` to `SplitTunnelSettings` trips three
  assertions. Adding it *and* updating the key pin to admit it still trips two
   — the shape rules do not depend on the pin. The name-disguised
  `perBinaryRouting?: {path, exit}[]`, with the pin updated too, still trips
  `ExitLivesOnlyOnTheGameGroup`. The structural rule holds against a field
  under any name.
* Dropping `.update(customerId)` from the exit handle — still keyed, still
  opaque, merely globally stable — fails `gives two customers different
  handles for the same machine`. Returning the node id instead fails that and
  `names no exit node -- no address, no hostname, no id -- anywhere in the
  payload`.

**Invariants 2–4 needed no argument:** `service/src/split_tunnel/` is
byte-identical to `9137eee` across all three branches, so `FlowKey`, the carry
ordering and both leak fixes are the code already proven on the wire. Only
`engines/{dns,ikev2,xray}.rs` changed in Rust.

Counts, against a baseline I re-established on `9137eee` rather than trusting
a note: 351 → 355 Rust, 218 → 244 vitest, 590 → 604 jest, 16 → 17 turbo. The
vitest and jest numbers are exactly the union of the branches' own additions,
which is the arithmetic that says nothing was lost in the merge.

**Trap worth writing down:** my first Rust baseline read 282, not 351, because
I had piped `cargo test` through `tail -60` and then counted `test result:`
lines in what survived. The count was an artifact of my own command. Capture
the whole run to a file and count from that; a plausible-looking number from a
truncated stream is worse than no number.

**Carried in from `exit-identity`, flagged rather than owned:** one additive
`ensure_env_key "EXIT_HANDLE_SECRET"` line in `installer/lib/panel.sh`, plus
...
## 2026-08-26 — CT enumerability: the capability landed, the migration did not

Went after the fleet republishing itself to Certificate Transparency.
Came back with the installer able to avoid it and **nothing changed on
production** — every remaining step is gated on a credential or a
provider console. Read `docs/node-enumerability-remediation.md`; it is
corrected against the running fleet now, and the "Measured state" section
at the top overrides the rest of the file where they disagree.

### Four premises in the old runbook were wrong

Worth knowing before anyone plans from it again:

* **Cloudflare is confirmed, not inferred.** `neoxify.site` is on
  Cloudflare nameservers, so `certbot-dns-cloudflare` is genuinely
  available. `neoxify.com` and `.net` are on Namecheap — but no node
  certificate is issued under either, so that blocks nothing.
* **The exposure spans a second domain.** Querying only `%.neoxify.site`
  showed six node names. `neoxify.com` carries four more, including a
  `us1` with no `.site` counterpart at all. Ten resolving names, not six.
* **The `*.neoxify.site` wildcard sitting in CT is not ours.** It is
  Cloudflare Universal SSL, issued by Google Trust Services and
  terminated at Cloudflare's edge. Its key has never been on our
  hardware, and no node holds a certificate with a wildcard SAN. The
  key-distribution trap the runbook warns about has not been sprung.
* **Every node runs IKEv2**, which quietly kills the cheerful path. The
  runbook says a node serving only the Xray TLS trio needs its A record
  solely for HTTP-01, so DNS-01 would let the name stop resolving. True
  of the code, vacuous in practice: there is no such node. IKEv2 clients
  dial the name and refuse an address.

### The consequence that matters

**DNS-01 and the nodes-only wildcard are not alternatives.** DNS-01 with
per-node names still writes the name into CT; it only removes the A
record requirement for renewal, and the A record has to stay anyway for
IKEv2. A wildcard is the only thing that keeps a name out of the log, and
Let's Encrypt issues wildcards only via DNS-01. So DNS-01 is the
prerequisite, not the choice. A plan that adopts DNS-01 and keeps
per-node names has spent the effort and bought nothing.

### Gotchas that cost time, or would have

* **Wiring only the Xray path would have been a silent no-op.** The first
  cut of the installer change did exactly that — helpers written, call
  sites untouched, 349 insertions and zero deletions. It looked done and
  changed nothing. Because every node runs IKEv2, leaving that path on
  per-node certificates republishes the hostname at every IKEv2 install
  and renewal regardless of what Xray does. Both consumers, or neither.
* **The key type is a live trap under a shared `--cert-name`.** Xray lets
  certbot default to ECDSA; IKEv2 forces RSA because Android rejects an
  ECDSA-signed AUTH payload. Two callers sharing one certificate name
  will flip it back and forth. `neoxify_acme_issue` strips key-type
  arguments from both, reuses rather than reissues, and warns on a
  non-RSA certificate.
* **A wildcard covers exactly one label.** `*.n.example.net` matches
  `a.n.example.net` and not `a.b.n.example.net`, and not the base itself.
  Tested all of those, plus `notn.example.net`, so a suffix match cannot
  false-positive.
* **`crt.sh` was returning 502 throughout.** Cert Spotter's free API
  answered every time and gives first/last issuance dates per name, which
  is what showed the renewal cadence. Worth reaching for first.
* **The nodes' own key type is a remote IKEv2 detector.** An RSA leaf on
  a node's TLS port means the IKEv2 installer reissued that name, since
  the Xray path would have left it ECDSA. That is how `de1` was
  classified without a login.

### Blocked, and on whom

`de1` and the panel refuse every key on this machine (`ovh_neo` reaches
fi1/fr1/ir1/sg1, `neo_tr1` reaches tr1). Not chased — no credential
guessing.

Everything left needs the owner:

1. **Namecheap** — delete the four `.com` node A records. This is the
   only step that *reduces* the live enumerable set rather than freezing
   it, and the evidence says nothing depends on them: they serve a
   certificate for a different name, no node renews a `.com` certificate
   any more, the shipped clients removed the `.com` origin, and the Tauri
   allowlist blocks `.com` outright.
2. **Hetzner and Linode consoles** — clear the three provider-default PTRs.
3. **A Cloudflare API token** scoped to `Zone:DNS:Edit` on `neoxify.site`.
   This one blocks everything else.

### The open question nobody has answered

**Does IKEv2 work with a wildcard certificate?** Windows validates the
server certificate against the name in the RAS entry; Android's
`Ikev2VpnProfile.Builder(server, server)` uses one string as both address
and remote identity. Neither is known to accept a wildcard SAN, and this
is now on the critical path because every node runs IKEv2.

Test it on `tr1` — it is not one of the two hardcoded API mirrors, so a
mistake there cannot take out the censorship fallback — with a real dial
from a Windows client and an Android handset. `swanctl --load-creds`
returning 0 proves nothing: the failure mode on record is Windows
silently discarding the IKE_AUTH response until the SA times out, which
reports as "terminated by the remote computer" and points at the wrong
thing.

If it does not work, the fallback is a split: the Xray TLS trio moves to
the wildcard, IKEv2 keeps a single-name certificate, and IKEv2 nodes stay
in CT. That is a gap to state, not to paper over.

### One correction to the framing

"The fleet republishes itself every ~90 days" is worth softening.
Re-issuing a certificate for a name **already** in the log gives an
enumerator nothing new — those names are permanent. The residual harm is
recency: a fresh entry proves the name is still live, where one last
certified in June looks decommissioned. Real, second-order, and never
worth an outage to fix. The first-order value is all in node seven
onwards, plus the `.com` deletion.

### Verified untouched

Panel serves, `/api/health` 200. Both API mirrors 200. Xray and
strongSwan active on all five reachable nodes. CT re-queried for both
domains after the session: no new names, none withdrawn — as expected,
since nothing was executed. Reputation baseline captured before the work
